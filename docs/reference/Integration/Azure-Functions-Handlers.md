...
```typescript
type TimerHandler = (context: AzureFunctionContext, timer: TimerInfo) => Promise<void> | void;
```
...
```typescript
type BlobHandler = (context: AzureFunctionContext, blob: BlobItem) => Promise<void> | void;
```
...
export const handler: DurableOrchestratorHandler<OrderWorkflow, string> = function* (context) {
  ...
  const isValid = yield context.callActivity<OrderWorkflow, boolean>(
    'ValidateOrder',
    context.input
  );
