...
User: c.model({
  ...
  allow.owner('id').read(['name', 'email']).update(['name']),

  // Managers can read salary but not SSN
  allow.groups(['manager']).read(['name', 'email', 'salary']),

  // HR can read everything except internal notes
  allow.groups(['hr']).read(['name', 'email', 'salary', 'ssn']),
...
export const event = defineEvents({
  ...
  ValidationRequested: configureEvent('ValidationRequested').withProcessor(
    async (context, event) => {
      // Validate data
  ...
  ValidationCompleted: configureEvent('ValidationCompleted').withProcessor(
    async (context, event) => {
      // Update final status
      await context.db.datasets.update(event.datasetId, {
...
// Number
a.number().required().min(0).max(100).integer().positive().negative().default(0);

// Boolean
a.boolean().required().default(false);
...
a.binary().required().maxSize(10_000_000); // 10 MB
```
...
```typescript
// Enum
a.enum(['pending', 'active', 'archived']).required().default('pending');
