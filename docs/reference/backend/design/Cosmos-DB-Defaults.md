...
backend.storage.database.attach(
  storage.cosmosDb().consistency('Strong') // For strict consistency
);
```
...
```typescript
backend.storage.database.attach(
  storage.cosmosDb().mode('Autoscale').maxThroughput(10000) // 10K max RU/s
);
```
...
backend.storage.database.attach(
  storage
    .cosmosDb()
    .encryption((enc) =>
      enc.customerManagedKeys().keyVault(process.env.KEY_VAULT_ID!).keyName('cosmos-encryption-key')
    )
);
```
...
backend.storage.database.attach(
  storage
    .cosmosDb()
    .firewall((fw) => fw.allowAzureServices(true).allowIps(['203.0.113.0/24']).denyAll())
);
```
...
export const data = defineStorage({
  Database: storage
    ...
    .backup(
      (backup) => backup.mode('Continuous').retention(90) // 90 days retention
    )
    .encryption((enc) => enc.customerManagedKeys().keyVault(process.env.KEY_VAULT_ID!))
    .firewall((fw) => fw.allowVnets([process.env.VNET_ID!]).denyAll()),
});
