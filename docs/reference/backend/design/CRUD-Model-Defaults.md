...
async function getUser(req: HttpRequest): Promise<HttpResponse> {
  ...
  const record = await cosmosContainer
    .item(
      req.params.id,
...
async function updateUser(req: HttpRequest): Promise<HttpResponse> {
  ...
  const existing = await cosmosContainer.item(req.params.id, req.params.id).read();
...
async function listUsers(req: HttpRequest): Promise<HttpResponse> {
  ...
  // 5. Execute query
  const { resources, hasMore, continuationToken } = await cosmosContainer.items
    .query(query, {
