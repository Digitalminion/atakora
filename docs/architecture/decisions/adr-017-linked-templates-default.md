...
This ADR represents a fundamental shift in how we generate and deploy ARM templates. While it adds complexity to the synthesis pipeline and deployment process, it's essential for creating a production-ready framework. The decision to make this the _only_ approach (not optional) ensures consistency and prevents fragmentation of the codebase.
