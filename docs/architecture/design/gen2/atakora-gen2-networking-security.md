...
class PrivateEndpointManager {
  createPrivateEndpoints(backend: Backend, config: NetworkingConfig): PrivateEndpoint[] {
    ...
    if (backend.cosmos) {
      endpoints.push(this.createPrivateEndpoint({
...
```typescript
function determineEndpointStrategy(environment: string, config?: NetworkingConfig): EndpointStrategy {
...
```typescript
function recommendNetworkTopology(orgSize: 'small' | 'medium' | 'enterprise'): EnvironmentIsolation {
  switch (orgSize) {
    case 'small':
