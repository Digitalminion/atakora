...
export const event = defineEvents({
  PostPublished: configureEvent('PostPublished').withProcessor(async (context, event) => {
...
export const schema = defineSchema({
  schema: a.schema({
    File: c
      ...
      .authorization((allow) => [allow.owner('uploadedBy'), allow.groups(['admin']).all()])
...
export const func = defineFunctions({
  GenerateUploadUrl: configureFunction('GenerateUploadUrl').withHandler(async (context, input) => {
...
export const event = defineEvents({
  FileUploaded: configureEvent('FileUploaded').withProcessor(async (context, event) => {
...
export const schema = defineSchema({
  schema: a.schema({
    ...
    Notification: c
      ...
      .authorization((allow) => [allow.owner('recipientId').read(), allow.groups(['admin']).all()])
...
export const event = defineEvents({
  ...
  DataValidated: configureEvent('DataValidated').withProcessor(async (context, event) => {
    if (event.isValid) {
      await context.db.datasets.update(event.datasetId, {
        status: 'processing',
  ...
  DataProcessed: configureEvent('DataProcessed').withProcessor(async (context, event) => {
    await context.db.datasets.update(event.datasetId, {
      status: 'ready',
    });
...
- [Getting Started Guide](./getting-started.md) - Deploy your first backend
- [Overview](./overview.md) - Package overview
- [Comparison Guide](./comparison.md) - Simple vs. full backend
