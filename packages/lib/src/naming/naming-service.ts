...
export class NamingService {
   ...
   */
  private readonly synthesisUniqueHash: string;
  ...
  constructor(seed?: string) {
    if (seed) {
      ...
      const uniqueInput = `${timestamp}-${randomBytes}`;
   ...
   * ```
   */
  public getUniqueHash(length: number = 8): string {
    if (length > this.synthesisUniqueHash.length) {
      throw new Error(
        `Requested hash length ${length} exceeds available hash length ${this.synthesisUniqueHash.length}`
      );
    }
   ...
   * ```
   */
  public getResourceHash(length: number = 8): string {
    if (length > this.synthesisUniqueHash.length) {
      throw new Error(
        `Requested hash length ${length} exceeds available hash length ${this.synthesisUniqueHash.length}`
      );
