...
export interface UserAssignedIdentityValue {
  // Empty object for requests
  // Azure may populate clientId and principalId in responses
}
 ...
 * ```
 */
export function createSystemAssignedIdentity(): ManagedServiceIdentity {
  return {
    type: ManagedIdentityType.SYSTEM_ASSIGNED,
  };
}
 ...
 * @internal
 */
export function validateManagedIdentity(identity?: ManagedServiceIdentity): void {
  if (!identity) {
    return;
  }

  const { type, userAssignedIdentities } = identity;
  ...
  if (
    (type === ManagedIdentityType.SYSTEM_ASSIGNED || type === ManagedIdentityType.NONE) &&
    userAssignedIdentities &&
    Object.keys(userAssignedIdentities).length > 0
  ) {
    throw new Error(
      `User-assigned identities should not be provided when identity type is '${type}'`
    );
  }

  // Validate resource ID format for user-assigned identities
  if (userAssignedIdentities) {
    for (const identityId of Object.keys(userAssignedIdentities)) {
