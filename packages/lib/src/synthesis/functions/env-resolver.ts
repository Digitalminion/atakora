...
export class EnvironmentResolver {
  ...
  resolveEnvironment(
    ...
    for (const [key, value] of Object.entries(functionEnv)) {
      if (typeof value === 'string') {
        ...
        const hasPlaceholders = this.placeholderPattern.test(value);
        this.placeholderPattern.lastIndex = 0; // Reset regex state

        if (hasPlaceholders) {
          // Resolve placeholders
          resolved[key] = this.resolvePlaceholders(
            value,
            appEnvironment,
            functionConfig.metadata.functionName
          );
        } else {
          // Keep literal value
          resolved[key] = value;
  ...
  private resolvePlaceholders(
    ...
    let result = template;
    const matches = template.match(this.placeholderPattern);
    ...
    for (const match of matches) {
      ...
      const placeholderName = match.slice(2, -1);

      // Check if value exists
      if (!(placeholderName in values)) {
        throw new EnvironmentResolutionError(
          `Environment variable '${placeholderName}' not provided in app.ts`,
          placeholderName,
          functionName
        );
      }
      ...
      // Resolve value
      const replacement = this.resolveValue(values[placeholderName]);
   ...
   * @internal
   */
  private resolveValue(value: string | IResourceReference): string {
    if (typeof value === 'string') {
      return value;
    }
  ...
  validateEnvironment(
    ...
    for (const value of Object.values(functionEnv)) {
      if (typeof value === 'string') {
        const placeholders = this.extractPlaceholders(value);
        placeholders.forEach((p) => required.add(p));
      }
    }
  ...
  resolveMultiple(
    ...
    for (const config of functionConfigs) {
      const functionEnv = this.resolveEnvironment(config, appEnvironment);
