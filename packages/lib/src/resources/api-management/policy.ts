...
import { PolicyFormat } from './types';
...
export class ArmApiManagementPolicy extends Resource {
   ...
   * Parent resource.
   */
  public readonly parent: IApiManagement | IApiManagementApi;
  ...
  /**
   * Type guard to check if parent is IApiManagementApi.
   */
  private isApiManagementApi(parent: any): parent is IApiManagementApi {
    return (
      parent &&
  ...
  public toArmTemplate(): object {
    ...
    if (this.isApiPolicy) {
      const apiParent = this.parent as IApiManagementApi;
      return {
      ...
      const serviceParent = this.parent as IApiManagement;
      return {
        type: this.resourceType,
        apiVersion: this.apiVersion,
