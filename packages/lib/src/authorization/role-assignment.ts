...
export class RoleAssignment extends Construct {
  ...
  public addDescription(description: string): void {
    throw new Error(
      'RoleAssignment is immutable: description must be set during construction. ' +
  ...
  public addCondition(condition: string, version: '2.0' = '2.0'): void {
    throw new Error(
      'RoleAssignment is immutable: condition must be set during construction. ' +
        `Received: "${condition}" (version: ${version}). ` +
   ...
   * @internal
   */
  public get armResource(): RoleAssignmentArm {
