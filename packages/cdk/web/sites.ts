 ...
 * ```
 */
export class Sites extends Construct implements ISite {
  /**
   * Underlying L1 construct.
   */
  private readonly armSite: ArmSites;
  ...
  constructor(scope: Construct, id: string, props: SitesProps) {
    ...
    this.armSite = new ArmSites(scope, `${id}Site`, {
    ...
    this.siteId = this.armSite.siteId;
    this.defaultHostName = this.armSite.defaultHostName;
  }
   ...
   * ```
   */
  public static fromSiteId(scope: Construct, id: string, siteId: string): ISite {
  ...
  private getParentResourceGroup(scope: Construct): IResourceGroup {
    ...
    throw new Error(
      'Sites must be created within or under a ResourceGroup. ' +
   ...
   */
  private resolveSiteName(id: string, props: SitesProps): string {
   ...
   */
  private resolveServerFarmId(serverFarmId: IServerFarm | string): string {
    if (typeof serverFarmId === 'string') {
      return serverFarmId;
    }

    // It's an IServerFarm interface - convert to ARM resourceId() expression
