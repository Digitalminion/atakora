...
import { SubscriptionDiagnosticSettingsArm } from './diagnostic-setting-subscription-arm';
 ...
 * ```
 */
export class SubscriptionDiagnosticSettings extends Construct {
  /**
   * Underlying L1 construct.
   * @internal
   */
  private readonly armDiagnosticSettings: SubscriptionDiagnosticSettingsArm;
  ...
  constructor(scope: Construct, id: string, props: SubscriptionDiagnosticSettingsProps) {
    ...
    // Store properties
    this.settingName = props.name;

    // Create underlying L1 resource
    this.armDiagnosticSettings = new SubscriptionDiagnosticSettingsArm(this, 'Resource', {
      diagnosticSettingName: props.name,
    ...
    this.settingId = this.armDiagnosticSettings.resourceId;
  }
  ...
  private validateParentScope(scope: Construct): void {
    ...
    if (!foundSubscriptionStack) {
      throw new Error(
        `SubscriptionDiagnosticSettings '${this.node.id}' must be created within a SubscriptionStack. ` +
          'Subscription diagnostic settings are subscription-scoped resources.'
      );
    }
  }
