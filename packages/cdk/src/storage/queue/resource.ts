...
export function createQueues(
  ...
  const queueStack = new QueueStack(scope, 'QueueStack', {
    storageAccount,
    queues: queueConfigs,
    cors: options?.cors,
  });
...
export function createStandardQueues(scope: Construct, storageAccount: IStorageAccount): QueueStack {
  return createQueues(scope, storageAccount, [
    ...
    { name: 'notifications', metadata: { purpose: 'notification-messages' } },
    { name: 'dead-letter', metadata: { purpose: 'failed-messages' } },
  ]);
}
 ...
 * ```
 */
export function createOrderQueues(scope: Construct, storageAccount: IStorageAccount): QueueStack {
  return createQueues(scope, storageAccount, [
    { name: 'orders', metadata: { purpose: 'new-orders' } },
    { name: 'order-processing', metadata: { purpose: 'order-processing' } },
    { name: 'order-completed', metadata: { purpose: 'completed-orders' } },
    { name: 'order-failed', metadata: { purpose: 'failed-orders' } },
  ]);
}
