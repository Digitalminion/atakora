...
exports.Sanitizer = exports.SizeValidator = exports.ContentTypeValidator = exports.JsonSchemaValidator = void 0;
...
class JsonSchemaValidator {
     ...
     * ```
     */
    static validateData(data, config) {
        const errors = [];
        ...
        return {
            ...
            problemDetails: valid
                ? undefined
                : problem_details_1.ProblemDetailsFactory.unprocessableEntity(config.errorMessage || 'Validation failed', errors),
        };
    }
    /**
     ...
     */
    static validateProperty(value, schema, field) {
        const errors = [];
        // Type validation
     ...
     */
    static validateFormat(value, format, field) {
        switch (format) {
            case 'email':
                if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value)) {
                    return {
...
class ContentTypeValidator {
     ...
     * ```
     */
    static validate(contentType, config) {
        // Check if required but missing
        if (config.required && !contentType) {
            return {
                valid: false,
                problemDetails: problem_details_1.ProblemDetailsFactory.badRequest('Content-Type header is required'),
            };
        }
        if (!contentType) {
            return { valid: true };
        ...
        const isAllowed = config.allowed.includes('*/*') ||
        ...
        if (!isAllowed) {
            return {
                valid: false,
                problemDetails: problem_details_1.ProblemDetailsFactory.custom(415, 'https://httpstatuses.io/415', 'Unsupported Media Type', config.errorMessage ||
                    `Content-Type must be one of: ${config.allowed.join(', ')}`, { receivedContentType: mediaType }),
            };
        }
        ...
        if (config.charset && charset !== config.charset.toLowerCase()) {
            return {
                valid: false,
                problemDetails: problem_details_1.ProblemDetailsFactory.badRequest(`Charset must be ${config.charset}`),
            };
        }
        return { valid: true };
    }
...
class SizeValidator {
     ...
     * ```
     */
    static validate(bodySize, headerSize, urlLength, config) {
        // Validate body size
        if (config.maxBodySize !== undefined &&
            ...
            return {
                valid: false,
                problemDetails: problem_details_1.ProblemDetailsFactory.custom(413, 'https://httpstatuses.io/413', 'Payload Too Large', `Request body must not exceed ${config.maxBodySize} bytes`, { maxSize: config.maxBodySize, actualSize: bodySize }),
            };
        }
        ...
        if (config.maxHeaderSize !== undefined &&
            ...
            return {
                valid: false,
                problemDetails: problem_details_1.ProblemDetailsFactory.custom(431, 'https://httpstatuses.io/431', 'Request Header Fields Too Large', `Request headers must not exceed ${config.maxHeaderSize} bytes`, { maxSize: config.maxHeaderSize, actualSize: headerSize }),
            };
        }
        // Validate URL length
        if (config.maxUrlLength !== undefined &&
            urlLength !== undefined &&
            urlLength > config.maxUrlLength) {
            return {
                valid: false,
                problemDetails: problem_details_1.ProblemDetailsFactory.custom(414, 'https://httpstatuses.io/414', 'URI Too Long', `Request URL must not exceed ${config.maxUrlLength} characters`, { maxLength: config.maxUrlLength, actualLength: urlLength }),
            };
        }
        return { valid: true };
    }
...
class Sanitizer {
     ...
     * ```
     */
    static applyToObject(obj, config) {
