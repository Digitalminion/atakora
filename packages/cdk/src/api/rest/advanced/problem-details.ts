 ...
 */
export class ProblemDetailsValidator {
  /**
  ...
  static isValid(problem: unknown): problem is ProblemDetails {
    ...
    if (typeof p.status !== 'number') return false;

    // Status must be valid HTTP status code
    if (p.status < 100 || p.status > 599) return false;
    ...
    if (p.errors !== undefined) {
      ...
      for (const error of p.errors) {
        ...
        if (!error.field || typeof error.field !== 'string') return false;
