 ...
 * ```
 */
export class ServerFarms extends Construct implements IServerFarm {
  /**
   * Underlying L1 construct.
   */
  private readonly armServerFarms: ArmServerFarms;
   ...
   * SKU name.
   */
  public readonly sku: ServerFarmSkuName;

  /**
   * SKU tier.
   */
  public readonly tier: ServerFarmSkuTier;
  ...
  constructor(scope: Construct, id: string, props?: ServerFarmsProps) {
    ...
    this.sku = props?.sku ?? ('B1' as ServerFarmSkuName);
    this.tier = this.inferTierFromSku(this.sku);

    // Default kind to Linux
    this.kind = props?.kind ?? ('linux' as ServerFarmKind);
    ...
    this.armServerFarms = new ArmServerFarms(scope, `${id}-Resource`, {
    ...
    this.planId = this.armServerFarms.planId;
  }
   ...
   * ```
   */
  public static fromPlanId(scope: Construct, id: string, planId: string): IServerFarm {
    // Extract plan name from resource ID
    const planNameMatch = planId.match(/\/serverfarms\/([^/]+)$/);
    if (!planNameMatch) {
      throw new Error(`Invalid Server Farm resource ID: ${planId}`);
    }
  ...
  private getParentResourceGroup(scope: Construct): IResourceGroup {
    ...
    throw new Error(
      'ServerFarms must be created within or under a ResourceGroup. ' +
   ...
   */
  private resolvePlanName(id: string, props?: ServerFarmsProps): string {
  ...
  private constructIdToPurpose(id: string): string | undefined {
    return utilConstructIdToPurpose(id, 'serverfarm', ['asp', 'aspsrvpl', 'plan']);
  }
   ...
   */
  private inferTierFromSku(skuName: ServerFarmSkuName): ServerFarmSkuTier {
    ...
    // Default to Basic
    return 'Basic' as ServerFarmSkuTier;
  }
   ...
   */
  private resolveReservedFlag(kind: ServerFarmKind, explicitReserved?: boolean): boolean {
