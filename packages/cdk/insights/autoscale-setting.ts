 ...
 * ```
 */
export class AutoscaleSettings extends Construct implements IAutoscaleSetting {
  private readonly armAutoscaleSetting: ArmAutoscaleSettings;
  ...
  constructor(scope: Construct, id: string, props: AutoscaleSettingsProps) {
    ...
    this.armAutoscaleSetting = new ArmAutoscaleSettings(scope, `${id}-Resource`, {
      name: this.name,
  ...
  private buildProfiles(props: AutoscaleSettingsProps): AutoscaleProfile[] {
  ...
  private resolveSettingName(id: string, props?: AutoscaleSettingsProps): string {
    if (props?.name) {
      return props.name;
    }
