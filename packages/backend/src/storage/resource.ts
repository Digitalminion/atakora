...
export const data = defineStorage({
  ...
  BlobStorage: storage
    ...
    .container('datasets', (c) => c.private().when(isProd, (c) => c.immutable(7)))
    .container('reports', (c) => c.private().deleteAfter(30))
    .container('uploads', (c) => c.private().deleteAfter(7))
    ...
    .container('temp', (c) => c.private().deleteAfter(1))
    .when(isProd, (b) =>
    ...
    .encryption((enc) =>
      enc
        ...
        .when(isProd, (e) =>
          e.keyVaultKey(process.env.STORAGE_ENCRYPTION_KEY!).requireInfrastructureEncryption()
        )
    )
    .network((net) =>
      net
    ...
    .softDelete((del) => del.enable().retainFor(isProd ? 30 : 7))
    .versioning((ver) => ver.enable(isProd))
    .changeFeed((feed) => feed.enable(isProd).retainFor(90)),
  ...
  Database: storage
    ...
    .when(
      isProd,
      (d) => d.throughput(400, 10000) // min, max RU/s
    )
    .consistency('Session')
    .when(isProd, (d) =>
    ...
    .backup((backup) =>
      backup
        ...
        .when(isProd, (b) => b.continuous('Continuous7Days'))
        .when(
          !isProd,
          (b) =>
            b
    ...
    .when(isProd, (d) =>
      d.analyticalStorage((analytics) => analytics.enable().schema('WellDefined'))
    )
    .network((net) =>
      net
        .when(isProd, (n) =>
        ...
        .when(!isProd, (n) => n.enablePublicAccess())
    )
    .encryption((enc) =>
      enc
        .keySource(isProd ? 'Microsoft.KeyVault' : 'Microsoft.Azure')
        .when(isProd, (e) => e.keyVaultKey(process.env.COSMOS_ENCRYPTION_KEY!))
    )
    .freeTier(!isProd)
    .performance((perf) =>
      perf
        .when(isProd, (p) => p.dedicatedGateway('Cosmos.D4s', 2))
        .queryMetrics()
        .indexing((idx) =>
          idx.automatic().mode('consistent').include('/*').exclude('/_etag/?', '/_attachments/*')
        )
    ),
});
