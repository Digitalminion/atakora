...
export const functions = defineCompute({
  ...
  FunctionApp: compute
    ...
    .scale((scale) =>
      scale.max(isProd ? 20 : 10).when(isProd, (s) =>
        s
          ...
          .rule('memory-scale', (rule) => rule.metric('MemoryPercentage').threshold(80).scaleBy(2))
          .rule('http-queue-scale', (rule) =>
            rule.metric('HttpQueueLength').threshold(100).scaleBy(1)
          )
      )
    )
    .performance((perf) =>
    ...
    .when(isProd, (a) => a.healthCheck((health) => health.path('/api/health').interval(30)))
    .deployment((deploy) =>
      deploy
        .runFromPackage()
        .when(isProd, (d) => d.slot('staging').stickySettings(['ENVIRONMENT', 'NODE_ENV']))
    )
    .when(isProd, (a) =>
      a.vnet((vnet) => vnet.enable().subnet(process.env.FUNCTION_SUBNET_ID!).routeAll())
    ),
});
