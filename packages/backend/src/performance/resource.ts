...
export const performance = definePerformance({
  ...
  CDN: perf
    ...
    .origin('function-app', (origin) =>
      origin.host(process.env.FUNCTION_APP_HOSTNAME!).http(80).https(443)
    )
    .rule(
      'static-assets',
      (rule) =>
        rule
          .order(1)
          .match((match) => match.path().beginsWith(['/assets/', '/static/']))
          .cacheExpiration('1.00:00:00') // 1 day
    )
    .rule('api-no-cache', (rule) =>
      rule
        .order(2)
        .match((match) => match.path().beginsWith(['/api/']))
        .bypass()
    )
    .compression((comp) =>
      comp
  ...
  Cache: perf
    ...
    .when(isProd, (cache) =>
      cache
        ...
        .backup((backup) => backup.rdb().frequency(60).maxSnapshots(1))
        .patching((patch) => patch.dayOfWeek('Sunday').startHour(2).window(5))
    )
  ...
  RateLimit: perf
    ...
    .perIp((limit) => limit.requests(200).window('1m').burst(250))
    ...
    .endpoint('POST /api/functions/transform-data', (limit) =>
      limit.requests(10).window('1m').burst(15)
    )
    .endpoint('POST /api/events/*', (limit) => limit.requests(50).window('1m').burst(75))
    .endpoint('POST /api/*', (limit) => limit.requests(50).window('1m').burst(75))
    .endpoint('PUT /api/*', (limit) => limit.requests(50).window('1m').burst(75))
    .endpoint('DELETE /api/*', (limit) => limit.requests(30).window('1m').burst(45))
    .headers((headers) =>
      headers
        .include()
